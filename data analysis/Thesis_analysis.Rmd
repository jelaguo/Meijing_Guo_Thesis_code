...
```
# Supplementary statistical test
```{r}
...
wga$type<-gsub("WGA-","",wga$type)
wga$type<-factor(wga$type, levels = c("EM","BS"))
wga$total_cpg<-(wga$total_cpg)^10
wga$total_gpc<-(wga$total_gpc)^10

mapped <- read.csv(paste0(OUTPUT_DIR, "/methylation used reads.csv"), header = TRUE)
mapped_wga<-mapped[mapped$sample %in% wga$sample,]
mapped_wga <- mapped_wga[match(wga$sample, mapped_wga$sample), ]
wga$normalised_cpg<- wga$total_cpg/mapped_wga$reads
wga$normalised_gpc<- wga$total_gpc/mapped_wga$reads

p_all <- ggplot(wga, aes(x = normalised_cpg, y = normalised_gpc)) +
  geom_jitter(
    aes(color = type),
    position = position_jitterdodge(jitter.width = 0.2, dodge.width = 0.75),  
    size = 1.5, alpha = 0.9
  ...
  scale_color_manual(values = c("EM"="#3B0F70FF","BS"="#F1605DFF"))+
  labs(
      title = paste0("Normalised", "\n", "WGA methods' methylation coverage"),
    x = "Normalised total CpG sites",
    y = "Normalised total GpC sites",
    color = "Method"
    ) +
...
ggsave(paste0(OUTPUT_DIR, "/meth_coverage_wga_normalised_new.png"), p_all, width = 5, height = 3.5)
print(p_all)
```
...
```
# 2.2 plot by condition comparison WGA-BS
```{r}
...
# 2.5 copy number calling (new version with all libraries separate for easier downstream access, doesn't affect the individual sample level GC bias correction)
```{r}
library(QDNAseq)
library(QDNAseq.hg38)
...
# segmentation for cell line specific data 
```{r}
library(QDNAseq)
library(QDNAseq.hg38)
library(Biobase)
...
```
# 2.6 copy number noise against read depth
```{r}
library(ggplot2)
library(ggrepel)
library(viridis)
...
# Result 2.7 Argeement ratio (100kb)
```{r}
library(GenomicRanges)
library(dplyr)
library(data.table)
...
# 2.8 correlation with OCUB-F reference (100kb)
```{r}
...
# 3.1 comparison between cell lines
```{r}
...
# 3.3 methylation pattern inspection (to validate cell line difference)
```{r}
samples<-basename(c(cov_testBS_cpg,cov_ctrlBS_cpg))
samples<-sub("_BS.*", "", samples)
samples<-sub("_Pool_", "-", samples)

clusterSamples(meth_bs, dist="correlation", method="ward.D", plot=TRUE)
hc = clusterSamples(meth_bs, dist="correlation", method="ward.D", plot=FALSE)
label_map <-setNames(samples, unlist(sample.id))
hc$labels <- label_map[hc$labels]

dend <- as.dendrogram(hc)
#install.packages('dendextend')
library(dendextend)
dend <- set(dend, "labels_cex", 1.2)  # Adjust for readability
# Assign two colors based on treatment
my.cols <- c("#3B0F70FF", "#F1605DFF")  
treatment_col <- my.cols[treatment + 1]  #  test=red, ctrl=blue

dend <- set(dend, "labels_col", treatment_col)
png_wgabs <- paste0(OUTDIR,"/methylation_profile_wgabs_dendrogram(methylkit).png")
png(png_wgabs, width = 800, height = 600) 
par(mar = c(10, 5, 4, 2))  
plot(dend, main = "Methylation pattern (WGA_BS)",  ylab = "Height")
legend("topright", legend = c("OCUB-F", "NA12878"), fill = my.cols, border = NA)
dev.off()
